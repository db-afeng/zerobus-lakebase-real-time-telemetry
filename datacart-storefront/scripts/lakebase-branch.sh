...
DEV_CONTEXT_ID_SCRIPT="$SCRIPT_DIR/dev-context-id.sh"
REFRESH_ONLY=false
SELF_TEST=false
WAIT_FOR_HOOK=false
...
build_lakebase_branch_name() {
    local git_user git_branch context_id raw prefix
    ...
    [[ "$context_id" =~ ^[0-9a-f]{8}$ ]] || {
        echo "Development context ID must be eight lowercase hexadecimal characters" >&2
        return 1
    }

    raw="dev-${git_user}-${git_branch}"
    prefix="${raw%-${context_id}}"
    prefix="${prefix:0:54}"
    prefix="${prefix%-}"
    printf '%s-%s\n' "$prefix" "$context_id"
}
...
env_matches_current_branch() {
    local expected_branch context_id
    [[ -f "$ENV_FILE" ]] || return 1
    context_id="$(bash "$DEV_CONTEXT_ID_SCRIPT")" || return 1
    expected_branch="$(
        build_lakebase_branch_name \
...
run_self_test() {
    local actual cursor collision_a collision_b long_name

    actual="$(build_lakebase_branch_name "Alex Feng" "feat/Lakebase Branch" "1234abcd")"
    [[ "$actual" =~ ^dev-alexfeng-feat-lakebase-branch-[0-9a-f]{8}$ ]] || {
        echo "Expected a sanitized, hashed branch name, got: $actual" >&2
        return 1
    }

    cursor="$(build_lakebase_branch_name "Alex Feng" "cursor/cc84ff57" "cc84ff57")"
    [[ "$cursor" == "dev-alexfeng-cursor-cc84ff57" ]] || {
        echo "Expected the Cursor ID to appear once, got: $cursor" >&2
        return 1
    }

    collision_a="$(build_lakebase_branch_name "Alex Feng" "feat/a_b" "aaaaaaaa")"
    collision_b="$(build_lakebase_branch_name "Alex Feng" "feat/a-b" "bbbbbbbb")"
    ...
    if build_lakebase_branch_name "!!!" "main" "1234abcd" >/dev/null 2>&1; then
...
DEV_CONTEXT_ID="$(bash "$DEV_CONTEXT_ID_SCRIPT")"
LAKEBASE_BRANCH="$(
    build_lakebase_branch_name "$GIT_USER_RAW" "$GIT_BRANCH_RAW" "$DEV_CONTEXT_ID"
)"
...
write_env_file() {
    ...
    trap 'rm -f "$tmp_file"' EXIT
    {
        ...
        echo "# Development context: $DEV_CONTEXT_ID"
