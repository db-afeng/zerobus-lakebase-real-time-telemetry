...
DATABRICKS_USER_ID="$(jq -er '.id' <<<"$CURRENT_USER_JSON")"
DB_USER="$(jq -er '.userName // .user_name' <<<"$CURRENT_USER_JSON")"
LAKEBASE_PROJECT_NAME="${LAKEBASE_PROJECT_NAME:-${LAKEBASE_PROJECT_PREFIX}-${DATABRICKS_USER_ID}}"
...
if ! databricks_cli postgres get-project "$PROJECT_PATH" -o json >/dev/null 2>&1; then
    ...
    echo "Deploy the bundle first or override LAKEBASE_PROJECT_NAME in lakebase.config." >&2
    exit 1
fi
